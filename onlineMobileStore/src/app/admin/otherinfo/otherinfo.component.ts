...
export class OtherinfoComponent {
  ...
  updateWebdata(): void {
    if (this.myForm.valid && this.editingWebdataId) {
      ...
      this.getProducts.updateWebData(this.editingWebdataId, updatedWebdata).subscribe(() => {
        this.isUpdated = true;
        setTimeout(() => {
  ...
  onFileChange(event: any): void {
    ...
    this.http.post('http://localhost:5000/upload-multiple', formData).subscribe((response: any) => {
